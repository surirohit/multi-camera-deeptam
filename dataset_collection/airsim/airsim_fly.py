...
X = 50          # final x coordinates
Y = -50         # length of path traversed along x
# The paramters H and X_step need to changed to collect more data
H = -10         # height of flight
X_step = 7.5
...
client.moveToPositionAsync(0, 0, H, V, 60, airsim.DrivetrainType.MaxDegreeOfFreedom, airsim.YawMode(False, YAW), -1, 0).join()
...
while x < X:
    ...
    client.moveToPositionAsync(x, y, H, V, delay_y, airsim.DrivetrainType.MaxDegreeOfFreedom, airsim.YawMode(False, YAW), -1, 0).join()

    x = x + X_step;

    printg("starting to move to (%3d, %3d)" % (x,y))
    client.moveToPositionAsync(x, y, H, V, delay_x, airsim.DrivetrainType.MaxDegreeOfFreedom, airsim.YawMode(False, YAW), -1, 0).join()

    printg("starting to move to (%3d, %3d)" % (x,y))
    client.moveToPositionAsync(x, 0, H, V, delay_y, airsim.DrivetrainType.MaxDegreeOfFreedom, airsim.YawMode(False, YAW), -1, 0).join()

    x = x + X_step;

    printg("starting to move to (%3d, %3d)" % (x,y))
    client.moveToPositionAsync(x, 0, H, V, delay_x, airsim.DrivetrainType.MaxDegreeOfFreedom, YawMode(False, YAW), -1, 0).join()
