...
class CameraIndexBag:
    ...
    def __init__(self, camera_idx, output_path):
        ...
        self.topics = []
        if RECORD_CAM_RGB:
            self.topics.append("/zed/zed_node/left/image_rect_color")
        if RECORD_CAM_INFO:
            self.topics.append("/zed/zed_node/left/camera_info")
        ...
        if self.idx % 2 == 0 and RECORD_CAM_DEPTH:
            self.topics.append("/zed/zed_node/depth/depth_registered")
    ...
    def save_image_msg(self, msg, curr_frame_time):
        ...
        if self.idx % 2 == 0 and RECORD_CAM_DEPTH:
            ...
            opencv_image = CvBridge().imgmsg_to_cv2(msg, "passthrough")
            depth_array = np.array(opencv_image, dtype=np.float32)
